...
@endphp
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <a href="{{route('add.photogalery')}}"><button type="button" class="float-right btn bg-teal-400 btn-labeled btn-labeled-left"><b><i class="icon-pen-plus"></i></b>Foto Galeri Ekle</button></a>
...
@php
    $i=1;
@endphp
                @foreach ($photo as $kategori )
                <tr>
                    <td>{{$i++}}</td>
                    <td>{{$kategori->title}}</td>


                    <td><img src="{{asset($kategori->photo)}}" width="100" alt=""></td>
                    <td>
                        @if ($kategori->type ==1)
                        <span class="badge badge-success">Büyük Foto</span>

                        @else
                        <span class="badge badge-success">Küçük Foto</span>
